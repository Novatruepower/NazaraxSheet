    ...
    async init() {
        try {
            await googleDriveFileFetcher.fetchGoogleSheetRange(googleDriveFileFetcher.My_Sheet.Races.gid, googleDriveFileFetcher.My_Sheet.Races.range).then(arr => {
                ...
                this._data['Stats'] = [...arr[0].filter(e => e != undefined), 'Mana', 'BaseHealth', 'BaseMana', 'RacialPower', 'BaseRacialPower']; // Copy header for 'Stats'
                ...
                this._data['Other'] = [health, 'BaseHealth', 'Mana', 'BaseMana', 'RacialPower', 'BaseRacialPower']; //By default 
                    ...
                    for (const option of passiveCopy.options) {
                        ...
                        if (option.options && option.options.values) {
                            ...
                            for (let i = 0; i < option.options.values.length; i++) {
                                ...
                                newOption.label = option.label.replace('%', `${Math.abs(newOption.value*100)}%`);
